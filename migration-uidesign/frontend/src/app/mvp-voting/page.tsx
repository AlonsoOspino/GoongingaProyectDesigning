...
export default function MvpVotingPage() {
  ...
  useEffect(() => { getMvpVoting({ cache: "no-store" }).then((data) => { setCampaign(data.campaign); setLoaded(true); }).catch(() => setError("MVP voting is unavailable right now.")); }, []);
