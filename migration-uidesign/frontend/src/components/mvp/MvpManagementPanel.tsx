...
export function MvpManagementPanel({ token }: { token: string }) {
  ...
  const load = async () => { try { const data = await getMvpManage(token); setCampaign(data.campaign); setMessage(data.reason || (data.campaign ? "" : "No finished Grand Finals campaign yet.")); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not load MVP campaign."); } };
  useEffect(() => { load(); }, []);
  async function status(status: "OPEN" | "CLOSED") { try { await updateMvpStatus(token, status); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not update voting."); } }
  async function upload(candidateId: number, file?: File) { if (!file) return; try { await uploadMvpImage(token, candidateId, file); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not upload image."); } }
  async function publish() { try { await publishMvpWinner(token); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not publish winner."); } }
